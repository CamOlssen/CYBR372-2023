...
public class FileEncryptor {
    ...
    public void run(String[] args) throws Exception {
        if (Objects.equals(args[0], "enc")) {
            ...
            //Generate secret keys and print them for user
            SecureRandom sr = new SecureRandom();
            byte[] key = new byte[16];
            sr.nextBytes(key); // 128 bit key
            vPNG vpng = new vPNG(seed);
            System.out.println("Seed: "+seed);
            byte[] initVector = new byte[16];
            initVector = vpng.next(initVector); // 16 bytes IV
            System.out.println("Random key=" + new String(Base64.getEncoder().encode(key)));
            System.out.println("initVector=" + new String(Base64.getEncoder().encode(initVector)));
     ...
     * @param key - the key used to encrypt the file
     * @param initVector - the initial vector used to encrypt the file
    ...
    public void dec(byte[] key, byte[] initVector, String inputDir, String outputDir) throws Exception {
        ...
        try (InputStream encrypted = Files.newInputStream(Paths.get(inputDir));
             CipherInputStream decryptStream = new CipherInputStream(encrypted, cipher);
             OutputStream decryptOut = new FileOutputStream(outputDir)) {
            try {
                final byte[] bytes = new byte[1024];
